...
//       DriveBase driveBase, Trajectory<SwerveSample> trajectory, boolean shouldMirror) {
...
//             .map(pose -> AllianceFlipUtil.apply(shouldMirror ? MirrorUtil.apply(pose) : pose))
//             .toArray(Pose2d[]::new));
//   }
//
//   @Override
...
//     }
//
//     var robotPose = PoseEstimator.getInstance().getEstimatedPose();
//
//     var sampleOpt = trajectory.sampleAt(timer.get(), false);
//     if (sampleOpt.isEmpty()) return;
//
//     var samplePose =
//         AllianceFlipUtil.apply(
//             shouldMirror ? MirrorUtil.apply(sampleOpt.get().getPose()) :
// sampleOpt.get().getPose());
//
//     //
